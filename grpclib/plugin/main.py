...
from typing import List, Iterable, NamedTuple, Tuple, Type, Iterator, Any
...
class Service(NamedTuple):
    name: str
    methods: List[Tuple[str, const.Cardinality, Type[Message], Type[Message]]]
...
def render(
    ...
    for service in services:
        ...
        with buf.indent():
            for (name, _, request_type, response_type) in service.methods:
                buf.add('')
                buf.add('@abc.abstractmethod')
                buf.add(
                    'async def {}(self, stream: {}.Stream[{}, {}]) -> None:',
                    name,
                    server.__name__,
                    request_type,
                    response_type,
        ...
        with buf.indent():
            ...
            with buf.indent():
                for method in service.methods:
                    name, cardinality, request_type, reply_type = method
                    full_name = '/{}/{}'.format(service_name, name)
                    method_cls: Type[client.ServiceMethod[Any, Any]]
...
def _get_proto(request: CodeGeneratorRequest, name: str):
...
def _type_name(proto_file, message_type) -> str:
...
def main() -> None:
    ...
    with os.fdopen(sys.stdout.fileno(), 'wb') as out:
        out.write(response.SerializeToString())  # type: ignore
